...
switch ( $option ) {
    ...
    $result = $usuarios->comprobarUsuario( $email, $password );
    if ( empty( $result ) ) {
        $res = array( 'tipo' => 'error', 'mensaje' => 'EMAIL O CONTRASEÑA INCORRECTOS' );
    } else {
    ...
    if ( $id_user == '' ) {
        $consult = $usuarios->comprobarUsuario( $correo, $password );
        if ( empty( $consult ) ) {
